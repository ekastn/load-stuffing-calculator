...
func (s *workspaceService) CreateWorkspace(ctx context.Context, req dto.CreateWorkspaceRequest) (*dto.WorkspaceResponse, error) {
	...
	ownerRoleID, err := lookupRoleID(ctx, s.q, types.RoleOwner.String())

	if err != nil {
		return nil, err
	}

	ws, err := s.q.CreateWorkspace(ctx, store.CreateWorkspaceParams{Type: "organization", Name: req.Name, OwnerUserID: userID})
	if err != nil {
		return nil, fmt.Errorf("failed to create workspace: %w", err)
	}

	_, err = s.q.CreateMember(ctx, store.CreateMemberParams{WorkspaceID: ws.WorkspaceID, UserID: userID, RoleID: ownerRoleID})
	if err != nil {
...
func (s *workspaceService) UpdateWorkspace(ctx context.Context, id string, req dto.UpdateWorkspaceRequest) (*dto.WorkspaceResponse, error) {
	...
	if req.OwnerUserID != nil {
		...
		if memberErr != nil {
			...
			if _, err := s.q.CreateMember(ctx, store.CreateMemberParams{WorkspaceID: ws.WorkspaceID, UserID: newOwnerID, RoleID: ownerRoleID}); err != nil {
