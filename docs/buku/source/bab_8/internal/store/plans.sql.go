...
VALUES ($1, 'draft')
RETURNING id, container_id, status, created_at, calculated_at, user_id
`
...
SELECT id, container_id, status, created_at, calculated_at, user_id FROM plans WHERE id = $1
`
...
SELECT p.id, p.container_id, p.status, p.created_at, p.calculated_at, p.user_id, c.name as container_name
FROM plans p
